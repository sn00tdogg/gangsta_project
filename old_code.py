...
def train(data, trials=10):
    test_accuracy = np.zeros(trials)
    for i in range(trials):
        print('Training network ', i + 1)
        random_state = 100 + i
        img_train, img_test, y_train, y_test = train_test_split(data.img, data.y, test_size=0.2,
                                                                random_state=random_state,
                                                                stratify=data.y)
        edges_train, edges_test = train_test_split(data.edges, test_size=0.2,
                                                   random_state=random_state,
                                                   stratify=data.y)
...
def test(data):
    print('Test accuracy: ', CNN.test(data.img, data.edges, data.y))
